...
for lat, lon, elev, name in zip(lt, ln, el, n):
    ...
    fgv.add_child(folium.CircleMarker(location=[lat, lon], radius=10, popup=folium.Popup(iframe), fill_color=color(elev),
                                     fill=True, color='blue', fill_opacity=0.7))

fgp = folium.FeatureGroup(name="Population")

fgp.add_child(folium.GeoJson(data=(open("world_data.json", 'r', encoding='utf-8-sig').read()),
